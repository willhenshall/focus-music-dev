import { X, Activity, Radio, AlertCircle, CheckCircle, Clock, TrendingUp, Zap, Move, Link, Layers, PlayCircle, Server, Gauge, Heart, Copy, Check, Music, ChevronDown, ChevronUp, Smartphone } from 'lucide-react';
...
export function AudioEngineDiagnostics({ metrics, onClose, engineType: _engineType = 'auto', isStreamingEngine = false, currentTrackInfo, prefetchTrackInfo }: AudioEngineDiagnosticsProps) {
  ...
  // Mobile stat row with larger touch targets
  const MobileStatRow = ({ label, value, valueClass = '' }: { label: string; value: string | number; valueClass?: string }) => (
  ...
  const MobileSection = ({ 
    ...
    <div className="border-b border-slate-200 last:border-0">
      <button
        ...
        className="w-full flex items-center justify-between p-3 bg-slate-50 active:bg-slate-100"
      >
        <div className="flex items-center gap-2">
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 bg-white z-50 flex flex-col overflow-hidden">
        {/* Mobile Header - Fixed */}
        <div className="flex-shrink-0 flex items-center justify-between px-3 py-2 bg-slate-900 text-white safe-area-inset-top">
          <div className="flex items-center gap-2">
            <Smartphone className="w-5 h-5 text-blue-400" />
            <span className="text-sm font-bold">Audio Diagnostics</span>
          </div>
          <div className="flex items-center gap-2">
            {/* Quick status badges */}
            <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${
              ...
              healthInfo.status === 'fair' ? 'bg-yellow-500' :
              'bg-red-500'
            }`}>
              {healthInfo.score}
            </span>
        ...
        <div className="flex-shrink-0 grid grid-cols-4 gap-1 p-2 bg-slate-100 border-b border-slate-200">
          <div className="bg-white rounded p-2 text-center">
            <div className={`text-lg font-bold ${getStatusColor(metrics.playbackState)}`}>
              {metrics.playbackState === 'playing' ? '▶' : metrics.playbackState === 'paused' ? '⏸' : '○'}
            </div>
            <div className="text-[9px] text-slate-500 uppercase">{metrics.playbackState}</div>
          </div>
          <div className="bg-white rounded p-2 text-center">
            <div className="text-lg font-bold text-slate-900">
              {hlsMetrics?.abr?.currentQualityTier?.charAt(0).toUpperCase() || 'A'}
            </div>
            <div className="text-[9px] text-slate-500 uppercase">
              {hlsMetrics?.abr?.currentQualityTier || 'Auto'}
            </div>
          </div>
          <div className="bg-white rounded p-2 text-center">
            <div className="text-lg font-bold text-slate-900">
              {Math.round(hlsMetrics?.bandwidthEstimate || 0) / 1000}k
            </div>
            <div className="text-[9px] text-slate-500 uppercase">BW</div>
          </div>
          <div className="bg-white rounded p-2 text-center">
            ...
            <div className="text-[9px] text-slate-500 uppercase">{metrics.connectionQuality}</div>
          </div>
        </div>

        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto">
          {/* ABR Quality Ladder - Always visible on mobile */}
          {hlsMetrics?.isHLSActive && hlsMetrics.levels.length > 0 && (
            <MobileSection 
              id="abr" 
              title="Quality Ladder" 
              icon={TrendingUp} 
              iconColor="text-purple-600"
              badge={hlsMetrics.abr ? {
                text: hlsMetrics.abr.abrState.toUpperCase(),
                color: hlsMetrics.abr.abrState === 'optimal' ? 'bg-green-100 text-green-700' :
            ...
            >
                ...
                {hlsMetrics.levels.map((level, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`text-[10px] ${isActive ? 'text-purple-200' : 'text-slate-500'}`}>
                        {Math.round(level.bitrate / 1000)}k
                      </div>
                      {isActive && <div className="text-[8px] mt-0.5">● ACTIVE</div>}
                    </div>
                  );
                })}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <MobileStatRow label="Bandwidth" value={`${Math.round((hlsMetrics.bandwidthEstimate || 0) / 1000)} kbps`} valueClass="text-purple-600" />
                  <MobileStatRow label="Recommended" value={hlsMetrics.abr?.recommendedQualityTier?.toUpperCase() || '-'} />
                  <MobileStatRow 
                    label="Match" 
                    value={hlsMetrics.abr?.recommendedQualityTier === hlsMetrics.abr?.currentQualityTier ? '✓' : '⚠'} 
                    valueClass={hlsMetrics.abr?.recommendedQualityTier === hlsMetrics.abr?.currentQualityTier ? 'text-green-600' : 'text-orange-600'}
                  />
                </div>
                <div>
                  <MobileStatRow label="Switches" value={hlsMetrics.abr?.totalLevelSwitches || 0} />
                  <MobileStatRow 
                    label="Direction" 
                    value={
                      hlsMetrics.abr?.isUpgrading ? '↑ Up' :
                      hlsMetrics.abr?.isDowngrading ? '↓ Down' : '→ Stable'
                    } 
                    valueClass={
                      hlsMetrics.abr?.isUpgrading ? 'text-green-600' :
                      hlsMetrics.abr?.isDowngrading ? 'text-orange-600' : 'text-blue-600'
                    }
                  />
                  <MobileStatRow 
                    label="Last Switch" 
                    value={hlsMetrics.abr?.timeSinceSwitch ? `${Math.round(hlsMetrics.abr.timeSinceSwitch / 1000)}s` : '-'} 
                  />
                </div>
              </div>
              ...
              {hlsMetrics.abr?.levelSwitchHistory && hlsMetrics.abr.levelSwitchHistory.length > 0 && (
                <div className="mt-3 pt-3 border-t border-slate-200">
                  <div className="text-xs font-bold text-slate-500 mb-2">Recent Switches</div>
                    ...
                    {[...hlsMetrics.abr.levelSwitchHistory].reverse().slice(0, 5).map((sw, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-slate-500">
                          {Math.round(sw.bandwidth / 1000)}k
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          <MobileSection 
            id="quality" 
            title="Quality & Buffer" 
            icon={Gauge} 
            iconColor="text-indigo-600"
            badge={hlsMetrics?.isHLSActive ? { text: 'HLS', color: 'bg-indigo-100 text-indigo-700' } : undefined}
          >
            <div className="grid grid-cols-2 gap-3">
              <div>
                <MobileStatRow label="Engine" value={isStreamingEngine ? 'Streaming' : 'Legacy'} valueClass="text-indigo-600" />
                <MobileStatRow label="Format" value={deliveryInfo.type === 'hls' ? 'HLS' : 'MP3'} />
                <MobileStatRow label="Mode" value={hlsMetrics?.isNativeHLS ? 'Native' : 'hls.js'} />
                <MobileStatRow label="Levels" value={hlsMetrics?.levels.length || '-'} />
              </div>
              <div>
                <MobileStatRow 
                  label="Buffer" 
                  value={`${hlsMetrics?.bufferLength?.toFixed(1) || 0}s / ${hlsMetrics?.targetBuffer || 30}s`} 
                  valueClass={
                    (hlsMetrics?.bufferLength || 0) / (hlsMetrics?.targetBuffer || 30) > 0.5 ? 'text-green-600' :
                    (hlsMetrics?.bufferLength || 0) / (hlsMetrics?.targetBuffer || 30) > 0.25 ? 'text-yellow-600' :
                    'text-red-600'
                  }
                />
                <MobileStatRow label="Segments" value={hlsMetrics?.bufferedSegments || 0} />
                <MobileStatRow label="Latency" value={`${Math.round(hlsMetrics?.latency || 0)}ms`} />
                <MobileStatRow label="Frag Success" value={(() => {
                  const total = (hlsMetrics?.fragmentStats.loaded || 0) + (hlsMetrics?.fragmentStats.failed || 0);
                  return total > 0 ? `${Math.round((hlsMetrics?.fragmentStats.loaded || 0) / total * 100)}%` : '100%';
              ...
              <div className="w-full bg-slate-200 rounded-full h-2">
                <div
                  className={`h-full rounded-full transition-all ${
                    (hlsMetrics?.bufferLength || 0) / (hlsMetrics?.targetBuffer || 30) > 0.5 ? 'bg-green-500' :
                    (hlsMetrics?.bufferLength || 0) / (hlsMetrics?.targetBuffer || 30) > 0.25 ? 'bg-yellow-500' :
                    'bg-red-500'
                  }`}
                  style={{ width: `${Math.min(((hlsMetrics?.bufferLength || 0) / (hlsMetrics?.targetBuffer || 30)) * 100, 100)}%` }}
                />
              </div>
            </div>
          </MobileSection>
          ...
          {/* Network Section */}
          <MobileSection id="network" title="Network & Health" icon={Activity} iconColor="text-green-600">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <MobileStatRow label="Connection" value={metrics.connectionQuality} valueClass={
                  metrics.connectionQuality === 'excellent' ? 'text-green-600' :
                  metrics.connectionQuality === 'good' ? 'text-blue-600' :
                  metrics.connectionQuality === 'fair' ? 'text-yellow-600' :
                  'text-red-600'
                } />
                <MobileStatRow label="Effective" value={metrics.effectiveType || 'unknown'} />
                <MobileStatRow label="Downlink" value={`${metrics.downlink || 0} Mbps`} />
                <MobileStatRow label="RTT" value={`${metrics.rtt || 0}ms`} />
              </div>
              <div>
                <MobileStatRow label="Health" value={`${healthInfo.score}/100`} valueClass={
                  healthInfo.status === 'excellent' ? 'text-green-600' :
                  healthInfo.status === 'good' ? 'text-blue-600' :
                  healthInfo.status === 'fair' ? 'text-yellow-600' :
                  'text-red-600'
                } />
                <MobileStatRow label="Circuit" value={metrics.circuitBreakerState} valueClass={getCircuitBreakerColor(metrics.circuitBreakerState).split(' ')[0]} />
                <MobileStatRow label="Stalls" value={metrics.stallCount} valueClass={metrics.stallCount > 0 ? 'text-orange-600' : ''} />
                <MobileStatRow label="Failures" value={metrics.failureCount} valueClass={metrics.failureCount > 0 ? 'text-red-600' : ''} />
              </div>
            </div>
          </MobileSection>

          {/* Playback Section */}
          <MobileSection id="buffer" title="Playback" icon={PlayCircle} iconColor="text-blue-600">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <MobileStatRow label="Time" value={`${formatTime(metrics.currentTime)} / ${formatTime(metrics.duration)}`} />
                <MobileStatRow label="Progress" value={`${Math.round((metrics.currentTime / (metrics.duration || 1)) * 100)}%`} />
                <MobileStatRow label="Load" value={`${metrics.loadDuration?.toFixed(0) || 0}ms`} />
              </div>
              <div>
                <MobileStatRow label="Volume" value={`${Math.round(metrics.volume * 100)}%`} />
                <MobileStatRow label="Buffered" value={`${metrics.bufferPercentage?.toFixed(0) || 0}%`} />
                <MobileStatRow label="Element" value={metrics.audioElement || '-'} />
              </div>
            </div>
          </MobileSection>

          {/* URLs Section */}
          <MobileSection id="urls" title="Track URLs" icon={Link} iconColor="text-slate-600">
            {currentTrackInfo?.trackName && (
              <div className="mb-2">
                <div className="flex items-center gap-2 mb-1">
                  <Music className="w-4 h-4 text-blue-600" />
                  <span className="text-xs font-medium text-slate-900 truncate">{currentTrackInfo.trackName}</span>
                </div>
                {currentTrackInfo.artistName && (
                  <span className="text-xs text-slate-500">{currentTrackInfo.artistName}</span>
                )}
              </div>
            )}
            <div className="bg-slate-50 rounded p-2 mb-2">
              <div className="text-[10px] text-slate-500 mb-1">Current URL</div>
              <div className="text-[10px] text-slate-700 break-all font-mono">
                {metrics.currentTrackUrl || 'None'}
              </div>
            </div>
            {metrics.prefetchedTrackUrl && (
              <div className="bg-slate-50 rounded p-2">
                <div className="text-[10px] text-slate-500 mb-1">Prefetch URL</div>
                <div className="text-[10px] text-slate-700 break-all font-mono">
                  {metrics.prefetchedTrackUrl}
                </div>
              </div>
        ...
        {/* Mobile Footer */}
        <div className="flex-shrink-0 flex items-center justify-between p-2 bg-slate-100 border-t border-slate-200 safe-area-inset-bottom">
          <span className="text-[10px] text-slate-500">
            Track: {metrics.currentTrackId || '-'}
          </span>
          <button
            onClick={exportDiagnostics}
            className="px-3 py-1.5 text-xs font-bold bg-slate-900 text-white rounded active:bg-slate-700"
          >
            Export JSON
          </button>
        </div>
