...
export async function selectNextTrack(
  ...
  const genreRules = ruleGroups?.flatMap(group =>
    group.rules?.filter(rule =>
      (rule.field === 'genre' || rule.field === "metadata->>'genre'") &&
      rule.operator === 'eq'
    ) || []
  ) || [];
  ...
  if (genreRules.length > 0) {
    ...
    tracksQuery = tracksQuery.eq('metadata->>genre', genreRule.value);
  }
  ...
  for (const track of tracks) {
    ...
    const enrichedMetadata = {
      ...
      bpm: track.tempo || track.metadata?.bpm,
    };
...
export async function selectNextTrackCached(
  ...
  let tracksQuery = supabase
    .from('audio_tracks')
    .select('id, metadata, speed, intensity, brightness, complexity, arousal, valence, tempo, duration_seconds, catalog, locked, track_user_genre_id, music_key_value, energy_set, track_id, energy_low, energy_medium, energy_high')
    .is('deleted_at', null);
  ...
  if (genreRules.length > 0) {
    tracksQuery = tracksQuery.eq('metadata->>genre', genreRules[0].value);
  }
