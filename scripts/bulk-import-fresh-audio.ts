...
async function createDatabaseRecord(fileName: string, fileSize: number): Promise<boolean> {
  ...
  const publicUrl = `${process.env.VITE_SUPABASE_URL}/storage/v1/object/public/audio-files/${fileName}`;
...
async function bulkImportAudio(audioDirectory: string) {
  ...
  if (!fs.existsSync(audioDirectory)) {
    ...
    console.log('   npx tsx scripts/bulk-import-fresh-audio.ts /path/to/audio/files');
    return;
  }
  ...
  if (!testSuccess) {
    ...
    console.log(`  Service Role Key (first 20 chars): ${process.env.VITE_SUPABASE_SERVICE_ROLE_KEY?.substring(0, 20)}...`);
