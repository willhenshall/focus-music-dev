...
const SOAK_TEST_DURATION = 2 * 60 * 60 * 1000; // 2 hours in milliseconds
const MIN_PLAY_DURATION = 30 * 1000; // 30 seconds minimum play time
const MAX_PLAY_DURATION = 5 * 60 * 1000; // 5 minutes maximum play time
const MIN_PAUSE_DURATION = 5 * 1000; // 5 seconds minimum pause time
const MAX_PAUSE_DURATION = 30 * 1000; // 30 seconds maximum pause time
...
class SoakTestRunner {
  ...
  async runSoakTest() {
    this.isRunning = true;
    this.log('START', `Starting 2-hour soak test`);
...
test.describe('Soak Test - 2 Hour Endurance', () => {
  test('should handle continuous usage for 2 hours without degradation', async ({ page }) => {
    test.setTimeout(SOAK_TEST_DURATION + 2 * 60 * 1000);

    console.log('\n🚀 Starting 2-Hour Soak Test\n');
