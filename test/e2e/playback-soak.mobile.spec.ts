...
/**
 * Mobile Playback Soak Tests
 *
 * Extended tests that simulate long-running audio sessions with repeated navigation
 * to catch audio restart bugs that occur during tab switching on mobile devices.
 *
 * These tests verify:
 ...
 *
 * Run with:
 *   TEST_USER_EMAIL=... TEST_USER_PASSWORD=... npm run e2e -- --project=mobile-chrome test/e2e/playback-soak.mobile.spec.ts
 */
  ...
  try {
    ...
    try {
      ...
      const hasAuthError = await page
        ...
        .catch(() => false);
      if (hasAuthError) {
        console.error("[SOAK-MOBILE] Authentication failed - invalid credentials");
        return false;
      }
      throw new Error("[SOAK-MOBILE] Dashboard did not load after sign in");
    }

    return true;
  } catch (error) {
    console.error("[SOAK-MOBILE] Failed to sign in as test user:", error);
    return false;
  }
}
...
async function navigateToChannels(page: Page): Promise<void> {
  ...
  const nav = page.locator('[data-testid="mobile-nav-channels"]');
...
async function navigateToProfile(page: Page): Promise<void> {
  ...
  const nav = page.locator('[data-testid="mobile-nav-profile"]');
  await nav.waitFor({ state: "visible", timeout: 5000 });
  await nav.tap();
...
async function navigateToSettings(page: Page): Promise<void> {
  ...
  const nav = page.locator('[data-testid="mobile-nav-settings"]');
  await nav.waitFor({ state: "visible", timeout: 5000 });
  await nav.tap();
...
async function navigateToSlideshow(page: Page): Promise<void> {
  ...
  const nav = page.locator('[data-testid="mobile-nav-slideshow"]');
  await nav.waitFor({ state: "visible", timeout: 5000 });
  await nav.tap();
...
async function startPlayback(page: Page): Promise<void> {
  ...
  // Wait for play/pause button and tap it
...
test.describe("Mobile Playback Soak Tests", () => {
  ...
  test("soak: Channels → Profile → Settings → Channels (5 cycles)", async ({ page }) => {
    const CYCLES = 5;
    console.log(`[SOAK-MOBILE] Starting Pattern A: ${CYCLES} cycles`);

    await startPlayback(page);
    await waitForMinPlaybackTime(page, MIN_PLAYBACK_TIME_BEFORE_CAPTURE);

    const baseline = await getPlaybackMetrics(page);
    console.log(`[SOAK-MOBILE] Baseline: sessionId=${baseline.sessionId}, trackUrl=${baseline.trackUrl?.slice(-30)}`);

    let previousTime = baseline.currentTime!;

    for (let cycle = 1; cycle <= CYCLES; cycle++) {
      console.log(`[SOAK-MOBILE] Cycle ${cycle}/${CYCLES}`);
    ...
    console.log(`[SOAK-MOBILE] ✅ Pattern A completed: ${CYCLES} cycles passed`);
  });
  ...
  test("soak: Channels → Slideshow → Profile → Channels (5 cycles)", async ({ page }) => {
    const CYCLES = 5;
    console.log(`[SOAK-MOBILE] Starting Pattern B: ${CYCLES} cycles`);
    ...
    console.log(`[SOAK-MOBILE] ✅ Pattern B completed: ${CYCLES} cycles passed`);
  });
  ...
  test("soak: Random navigation pattern (15 cycles)", async ({ page }) => {
    const CYCLES = 15;
    console.log(`[SOAK-MOBILE] Starting Pattern C (Random): ${CYCLES} cycles`);
    ...
    for (let cycle = 1; cycle <= CYCLES; cycle++) {
      ...
      console.log(`[SOAK-MOBILE] Cycle ${cycle}/${CYCLES} → ${tab}`);
      await NAV_MAP[tab](page);
    ...
    console.log(`[SOAK-MOBILE] ✅ Pattern C (Random) completed: ${CYCLES} cycles passed`);
  });
  ...
  test("soak: Heavy mixed navigation (30 cycles)", async ({ page }) => {
    const CYCLES = 30;
    console.log(`[SOAK-MOBILE] Starting Pattern D (Heavy): ${CYCLES} cycles`);
    ...
    for (let cycle = 1; cycle <= CYCLES; cycle++) {
      ...
      if (cycle % 10 === 0) {
        console.log(`[SOAK-MOBILE] Progress: ${cycle}/${CYCLES}`);
      }
    ...
    console.log(`[SOAK-MOBILE] Total playback time during test: ${totalPlaybackTime.toFixed(2)}s`);
    console.log(`[SOAK-MOBILE] ✅ Pattern D (Heavy) completed: ${CYCLES} cycles passed`);
  });
  ...
  test("soak: Verify metrics remain non-null throughout navigation", async ({ page }) => {
    console.log(`[SOAK-MOBILE] Starting metrics stability verification`);
    ...
    for (const tab of ALL_TABS) {
      ...
      console.log(`[SOAK-MOBILE] ${tab}: all metrics valid ✓`);
    }

    console.log(`[SOAK-MOBILE] ✅ Metrics stability verification passed`);
  });
});
...
// ============================================================================
test.describe("Mobile Soak - Configuration Verification", () => {
  test.use({
  ...
  test("verifies test environment configuration", async ({ page }) => {
    console.log("========================================");
    console.log("MOBILE SOAK TEST CONFIGURATION");
    console.log("========================================");

    const viewportSize = page.viewportSize();
    console.log(`[CONFIG] Viewport: ${viewportSize?.width}x${viewportSize?.height}`);
